...
sprite.player.src = "../assests/Poyo.png";
sprite.platform.src = "../assests/platform.png";
sprite.ground.src = "../assests/tile_0069.png";
sprite.background.src = "../assests/background.png";

//variable to extract the number of images loaded
let imagesLoaded =0;
const totalImages = Object.keys(sprite).length;

//func to load images
function onImageLoad(){
    imagesLoaded++;
    // start the game looop only when four iamge are loaded
    if(imagesLoaded===totalImages){
        gameLoop();
    }
}

// add load event listioner to each image
for(let key in sprite){
    sprite[key].addEventListener('load', onImageLoad);
    sprite[key].addEventListener('error', 
        ()=>{
            console.error(`Failed to load image:${sprite[key].src}`);
        }
    );
}
